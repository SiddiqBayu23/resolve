...
const CategoriesView = ({ products, handleNavigateDetail, showFilters, toggleFilter, handleFilterChange, filters, handleRemoveFilter }) => (
  ...
  {products.map((product) => (
    <div className="rounded-md border border-gray-400 overflow-hidden bg-white px-4 py-6" key={product.product_id} onClick={() => handleNavigateDetail(product.id)}>
      <div className="aspect-w-3 aspect-h-4 sm:aspect-h-5 md:aspect-h-6 lg:aspect-h-7 overflow-hidden">
        <img src={product.imageUrl} alt="Product Image" className='object-cover' />
      </div>
      <div className='flex flex-col gap-2 mt-4'>
        <p className='text-base sm:text-lg font-semibold font-manrope text-pink-300 truncate'>{product.product_name}</p>
        <p className='font-manrope text-gray-400 text-sm sm:text-base line-clamp-2'>{product.product_subname}</p>
