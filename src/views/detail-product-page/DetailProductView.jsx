...
const DetailProductView = ({ product, tab, handleSetTab, recommended, like, handleNavigateDetail }) => (
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-8">
    {recommended.map((product) => (
      <div className="rounded-md border border-gray-400 overflow-hidden bg-white px-4 py-6" key={product.product_id} onClick={() => handleNavigateDetail(product.id)}>
        ...
        <div className="flex flex-col gap-2 mt-4">
          ...
          <p className="font-manrope text-gray-400 text-xs sm:text-sm md:text-base line-clamp-2">{product.product_subname}</p>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 mt-8">
        {like.map((product) => (
            <div className="rounded-md border border-gray-400 overflow-hidden bg-white px-4 py-6">
            <div className="h-48 sm:h-56 md:h-64 lg:h-72 overflow-hidden">
              <img src={product.imageUrl} alt="Product Image" className="block mx-auto w-full h-full object-cover" />
            </div>
            <div className="flex flex-col gap-2 mt-4">
